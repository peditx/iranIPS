...
check_depends() {
	...
	if [ "$tables" == "iptables" ]; then
		for depends in "iptables-mod-tproxy" "iptables-mod-socket" "iptables-mod-iprange" "iptables-mod-conntrack-extra" "kmod-ipt-nat"; do
			[ -s "${file_path}/${depends}${file_ext}" ] || echolog "$tablesThe basic dependencies for transparent proxy $depends Not installed..."
		done
	else
		for depends in "kmod-nft-socket" "kmod-nft-tproxy" "kmod-nft-nat"; do
			[ -s "${file_path}/${depends}${file_ext}" ] || echolog "$tablesTransparent proxy basic dependencies. $depends Not installed..."
		done
	fi
}
...
ln_run() {
	...
	if [  "${file_func%%/*}" != "${file_func}" ]; then
		...
		[ -x "${file_func}" ] || echolog "  - $(readlink ${file_func}) No execution permission, unable to start：${file_func} $*"
	fi
	#echo "${file_func} $*" >&2
...
run_socks() {
	...
	if [ -n "$server_host" ] && [ -n "$port" ]; then
		...
		[ $? != 0 ] && {
			echolog "  - Socks节点：[$remarks]${server_host} The server address is invalid, unable to start！"
			return 1
		}
		tmp="${server_host}:${port}"
	else
		error_msg="For some reason, the related configuration of this Socks service is disconnected, and the startup is aborted！"
	fi
...
run_global() {
	...
	local msg="${TUN_DNS} （Direct DNS connection：${AUTO_DNS}"
...
start_crontab() {
	...
	[ -f "/tmp/lock/${CONFIG}_cron.lock" ] && {
		...
		echolog "Currently set to run automatically as a scheduled task, no reconfiguration of the scheduled task is required. "
		return
	}
	...
	if [ -n "$stop_week_mode" ]; then
		...
		echolog "Configure the scheduled task: Automatically close the service. "
	fi
	...
	if [ -n "$start_week_mode" ]; then
		...
		echolog "Configure the scheduled task: Automatically start the service. "
	fi
	...
	if [ -n "$restart_week_mode" ]; then
		...
		echolog "Configure the scheduled task: Automatically restart the service. "
	fi
	...
	if [ "$autoupdate" = "1" ]; then
		...
		echolog "Configure the scheduled task: Automatically update the rules. "
	fi
	...
	for item in $(uci show ${CONFIG} | grep "=subscribe_list" | cut -d '.' -sf 2 | cut -d '=' -sf 1); do
		if [ "$(config_n_get $item auto_update 0)" = "1" ]; then
			...
			echolog "Configure the scheduled task: Automatically update【$remark】Subscribe. "
		fi
	done
	...
	if [ "$ENABLED_DEFAULT_ACL" == 1 ] || [ "$ENABLED_ACLS" == 1 ]; then
		...
		echolog "Running in non-proxy mode, only scheduled tasks for starting and stopping the service are allowed. "
	fi
...
add_ip2route() {
	...
	[ -z "$ip" ] && {
		echolog "  - Unable to resolve[${1}]，Failed to add route table！"
		return 1
	}
	local remarks="${1}"
	...
	if [ -n "${gateway}" ]; then
		...
		echolog "  - [${remarks}]Add to interface[${device}]Route table added successfully！"
	else
		echolog "  - [${remarks}]Added to interface[${device}]Route table failed！The reason is that it cannot be found[${device}]Gateway。"
	fi
}
			...
			[ -n "$node" ] && {
				...
				if [ "$node" = "default" ]; then
					if [ -n "${GLOBAL_node}" ]; then
						...
						echolog "  - Global node is not enabled, skipping【${remarks}】"
					fi
				else
...
start() {
	pgrep -f /tmp/etc/passwall2/bin > /dev/null 2>&1 && {
		echolog "The program has already started. Please stop it first and then restart it!"
		stop
	}
	...
	if [ "$use_nft" == 0 ]; then
		if [ -n "$(command -v iptables-legacy || command -v iptables)" ] && [ -n "$(command -v ipset)" ] && [ -n "$(dnsmasq --version | grep 'Compile time options:.* ipset')" ]; then
			...
			echolog "The system does not have iptables or ipset installed, or Dnsmasq does not have ipset support enabled. Transparent proxy using iptables+ipset is not available!"
			if [ -n "$(command -v fw4)" ] && [ -n "$(command -v nft)" ] && [ -n "$(dnsmasq --version | grep 'Compile time options:.* nftset')" ]; then
				echolog "Detected fw4, using nftables for transparent proxy."
				USE_TABLES="nftables"
				nftflag=1
		...
		if [ -n "$(dnsmasq --version | grep 'Compile time options:.* nftset')" ]; then
			...
			echolog "The Dnsmasq package does not meet the requirements for nftables transparent proxy. To use it, please ensure that the Dnsmasq version is 2.87 or higher and nftset support is enabled."
		fi
	fi
	...
	[ "$USE_TABLES" = "nftables" ] && {
		...
		[ "$(expr $dnsmasq_version \>= 2.90)" == 0 ] && echolog "The Dnsmasq version is below 2.90. It is recommended to upgrade to version 2.90 or higher to avoid potential Dnsmasq crash issues in certain situations!"
	}
...
stop() {
	...
	echolog "Clearing and closing related programs and caches completed. "
	exit 0
}
