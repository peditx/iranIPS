#!/bin/sh
# Passwall 1 + 2 Installer for PeDitXOS (APK + OPKG)

echo ">>> Installing Passwall 1 + 2..."
...
pkg_install dnsmasq-full unzip luci-app-passwall luci-app-passwall2 ca-bundle kmod-tun > /dev/null 2>&1

# Verify
FAIL=0
[ -f /etc/init.d/passwall ]  && echo "[*] Passwall 1 OK"  || { echo "[!] Passwall 1 failed"; FAIL=1; }
[ -f /etc/init.d/passwall2 ] && echo "[*] Passwall 2 OK"  || { echo "[!] Passwall 2 failed"; FAIL=1; }
[ $FAIL -eq 1 ] && exit 1
